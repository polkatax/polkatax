...
export const calculateRewardSummary = (rewards: Reward[]): RewardSummary => {
  ...
  for (const year of [currentYear, currentYear - 1]) {
    ...
    const temp = rewards.filter(r => r.timestamp >= beginning && r.timestamp <= end)
    resultsPerYear.push({ year, ...temp.reduce<{ amount: number, fiatValue?: number}>(
      (acc, reward) => {
        return {
  ...
  return {
    ...
    fiatValue: resultsPerYear.reduce<number | undefined>((prev, curr) => (curr.fiatValue !== undefined && prev !== undefined) ? prev + curr.fiatValue : undefined, 0),
