...
export const extractStakingRewardsPerYear = (rewards: Rewards | undefined, year: number): StakingRewardsPerYear | undefined => {
    if (!rewards) {
      return undefined
    }
    ...
    return {
      ...
      dailyValues: Object.fromEntries(
        Object.entries(rewards.dailyValues).filter(([key]) => key >= beginningOfYearFormatted && key <= endOfYearFormatted)
      ),
      values: rewards.values.filter(v => v.timestamp >= beginning && v.timestamp <= end),
